...
def main():
    ...
    import time 
    total_duration = []
    # for i, batch_data in tqdm(enumerate(dataset), disable=(local_rank != 0)):
    ...
    for index in tqdm(range(int(math.ceil(len(dataset) / world_size))), disable=local_rank != 0):
        ...
        if args.i2v:
            ...
            # Process the image
            image = batch['image'].squeeze(0).unsqueeze(0).unsqueeze(2).to(device=device, dtype=torch.float32)

            # Encode the input image as the first latent
            initial_latent = pipeline.vae.encode_to_latent(image).to(device=device, dtype=torch.float32)
            initial_latent = initial_latent.repeat(args.num_samples, 1, 1, 1, 1)


            sampled_noise = torch.randn(
                [args.num_samples, args.num_output_frames - 1, 16, 60, 104], device=device, dtype=torch.float32
            )
        else:
