...
def store_video_arrays_to_lmdb(env, arrays_dict, start_index=0):
    """
    Store rows of multiple numpy arrays in a single LMDB.
    Each row is stored separately with a naming convention.
    """
...
def process_data_dict_videos(data_dict, seen_prompts):
    output_dict = {}

    all_videos = []
    all_noise = []
    all_prompts = []
    for prompt, values in data_dict.items():
        if prompt in seen_prompts:
            continue
        else:
            seen_prompts.add(prompt)
        
        video = values["video"].half().numpy()
        noise = values["noise"].half().numpy()
    ...
    if len(all_videos) == 0:
        return {"videos": np.array([]), "noise": np.array([]), "prompts": np.array([])}

    all_videos = np.concatenate(all_videos, axis=0)
    all_noise = np.concatenate(all_noise, axis=0)
    

    output_dict['video'] = all_videos
    output_dict['noise'] = all_noise
    output_dict['prompts'] = np.array(all_prompts)
...
def retrieve_row_from_video_lmdb(lmdb_env, array_name, dtype, row_index,shape=None):
    """
    Retrieve a specific row from a specific array in the LMDB.
    """
    data_key = f'{array_name}_{row_index}_data'.encode()
