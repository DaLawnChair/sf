"""
This is purely a test to see if the progressive works at changning the # of chunks that are denoised at once.
...
# save and view metrics calculated
results = {}
results["total_duration"] = sum(total_duration)
results["average_duration_per_video"] = sum(total_duration)/len(total_duration)
results["actual_durations"] = total_duration
results["max_memory_reserved"] = round(torch.cuda.max_memory_reserved()/ (1024**3),4)
results["max_memory_allocated"] = round(torch.cuda.max_memory_allocated()/ (1024**3),4)
