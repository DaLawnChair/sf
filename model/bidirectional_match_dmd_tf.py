...
class BidirectionalMatchDMDTeacherForcing(SelfForcingModel):
    def __init__(self, args, device):
        ...
        self.original_denoising_step_list = getattr(args, "denoising_step_list", [])
    ...
    def generator_loss(
        ...
        with torch.no_grad():
            ...
            generated_video_latents_bidirectional, bd_denoised_timestep_from , bd_denoised_timestep_end, bd_stored_generations = self.inference_pipeline.inference_with_trajectory(
                noise=sampled_noise,
                **conditional_dict,
        ...
        if self.teacher_forcing:
            ...
            if self.tf_noised_latent:
                ...
                raise NotImplementedError("logic for this is broken after addition of timestep scaling")
        ...
        def compare_generations(causal: torch.Tensor, bidirectional:list[torch.Tensor], end_timestep):
            """
            Compares the chunk generated by the causal model versus the bidirectional one, using end_timestep as the gauge for the 
            timestep to compare with
            """    
            loss_total = []
            end_timestep_indicies = []
            if len(bidirectional) == 1:
                bi_gen_latent_index = 0
            for chunk_index in range(num_blocks):
                
                # find what timestep to use for this chunk based on the end_timestep for it
                timestep_pos = torch.argwhere(self.inference_pipeline.denoising_step_list == end_timestep[chunk_index].item()).item()
                end_timestep_indicies.append(timestep_pos)
                assert timestep_pos != -1, "wrong index calculation for timestep_pos"
                
                causal_gen_chunk = causal[:, chunk_index*self.num_frame_per_block:(chunk_index+1)*self.num_frame_per_block, ...]
                
                bi_gen = bidirectional[bi_gen_latent_index][timestep_pos]
                bi_gen_chunk = bi_gen[:, chunk_index*self.num_frame_per_block:(chunk_index+1)*self.num_frame_per_block, ...]
            
                loss = F.mse_loss(causal_gen_chunk, bi_gen_chunk.detach(), reduction="none")
                loss_total.append(loss)
            return torch.stack(loss_total, dim=1).flatten(1,2), end_timestep_indicies
        

        recreation_losses, end_timestep_indicies = compare_generations(
            causal=tf_pred,
            bidirectional=bd_stored_generations,
            end_timestep=end_timestep
        )
        recreation_losses = recreation_losses.mean(dim=(2,3,4)) # get mse loss for each chunk, shape [B, num_chunks]

        if self.use_trainer_weighted_loss:
            assert end_timestep is not None, "end_timestep should not be None when scale_earlier_chunks is True"
            # repeated_end_timestep = torch.repeat_interleave(denoising_step_list_pos, self.num_frame_per_block, dim=0)
