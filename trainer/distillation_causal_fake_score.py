...
class Trainer:
    def __init__(self, config):
        ...
        assert self.model.use_causal_fake_score, "usage of this trainer should have use_causal_fake_score=True"
        assert self.model.fake_score.is_causal, f"before fsdp_wrap on fake_score, the model is not causal self.model.fake_score.is_causal={self.model.fake_score.is_causal}"
    ...
    def train(self):
        ...
        while True:
            ...
            for accumulation_step in range(self.gradient_accumulation_steps):
                print(f"======== step critic: {self.step+1} == On accumulation_step: {accumulation_step+1} ========")
